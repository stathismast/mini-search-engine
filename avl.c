...
int insertToAVL(int id, double score, AVLTree ** tree){
	...
	if(id < (*tree)->id){	//If the given id is less that the id of the current node
		nodeAdded = insertToAVL(id, score, &(*tree)->left);
		...
		nodeAdded = insertToAVL(id, score, &(*tree)->right);
...
void avlToSearchInfoArray(AVLTree * tree, SearchInfo ** array, int * start){
	...
	array[*start]->id = tree->id;
	array[*start]->score = tree->score;
	(*start)++;
	avlToSearchInfoArray(tree->left, array, start);
	avlToSearchInfoArray(tree->right, array, start);
}
